...
export const exportNewsData = async (
  format: "json" | "csv" = "json"
): Promise<Blob> => {
  try {
    const response = await api.get(`/admin/news/export`, {
      params: { format },
      responseType: "blob",
    });
...
export const importNewsData = async (
  file: File
): Promise<{
  message: string;
  report: {
  ...
  try {
    ...
    const response = await api.post(`/admin/news/import`, formData, {
      headers: {
